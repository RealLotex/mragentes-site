...
- **👁️ REGLA DE USO (2026-08-21):** la visión es la herramienta de verificación visual PREDETERMINADA. Es más barata que razonar sobre HTML/CSS o asumir renders. Ante cualquier duda visual (render de slide, screenshot, comparación de PNGs, UI, logo) → `image`/`images` (batch hasta 10). Ver AGENTS.md → "ABUSÁ de la visión barata".
...
- ⚠️ **2026-08-21 DUPLICADO FB por proceso cortado + dedup roto (`cancer-vacuna-ia`)**. El carrusel de la vacuna de Moderna salió 2 veces en FB. Cadena exacta: 1) el push a GitHub falló (repo desincronizado) pero el flujo siguió y publicó el álbum en FB; 2) el proceso quedó esperando la URL pública para IG y fue matado → state.json NUNCA se registró; 3) el reintento vio state.json vacío, intentó detectar el post con `GET /{page_id}?fields=id,message` (endpoint INCORRECTO: eso es el objeto page, no el feed → error #100) y el `except` se tragó el error → publicó de nuevo. **FIX aplicado (`flow.py` + `cli.py`)**: registrar el id de FB en state.json INMEDIATAMENTE después de publicar (no al final del flujo), así un proceso muerto deja registro y el reintento aborta. **Regla: cualquier publicación multi-paso debe registrar por plataforma apenas publica, y un chequeo de dedup que falla debe ABORTAR, nunca seguir.**
- ⚠️ **El feed de FB NO es consultable con el token actual**: `GET /{page_id}/feed` da `(#10) requires pages_read_engagement` (o Page Public Content Access). El token tiene `pages_manage_posts` + `pages_read_engagement` según debug_token 2026-08-10, pero el endpoint de feed lo rechaza igual. Consecuencia: NO se puede deduplicar leyendo el feed; el state.json temprano es la única defensa fiable.
